...
class Matrix_Window(Window):
    ...
    def setup_transformation_buttons(self): # draw matrix buttons        
        ...
        tk.Button(self.panel, text="A + B", width=7, height=1, command=lambda: #add button
            (gmc.log_gmc([("", self.mat[0]), ("+", self.mat[1]), ("=", Matrix.add_subtract_matrice(self.mat[0], self.mat[1]))]))).place(
            x = 400, y = c.MATRIX_Y_BASE + 15, anchor="center")
        tk.Button(self.panel, text="A - B", width=7, height=1, command=lambda: #subtract button
            (gmc.log_gmc([("", self.mat[0]), ("-", self.mat[1]), ("=", Matrix.add_subtract_matrice(self.mat[0], self.mat[1], True))]))).place(
            x = 400, y = c.MATRIX_Y_BASE + 45, anchor = "center")
        ...
        for i in range(2): 
            ...
            tk.Button(self.panel, text="Multiply By", width=12, height=1, command=lambda i=i:
                (gmc.log_gmc([(f"{button_input_fields[i].get()} x", self.mat[i]), ("=", self.mat[i].scalar_multiply(float(button_input_fields[i].get())))]))).place(
            ...
            tk.Button(self.panel, text="Raised By", width=12, height=1, command=lambda i=i:
                (gmc.log_gmc([("", self.mat[i]), (f"^({button_input_fields[i + 2].get()}) =", self.mat[i].to_pow(float(button_input_fields[i + 2].get())))]))).place(
            ...
            (gmc.log_gmc([("Det", self.mat[i], f"= {self.mat[i].determinant():g}" if not isinstance(self.mat[i].determinant(), bool) else False)]))).place(
